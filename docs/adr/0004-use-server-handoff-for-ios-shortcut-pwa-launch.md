...
Share Handoffは30分で期限切れになる。同一ユーザーから新しいShare Handoffが作成された場合、既存のpendingをsupersedeする。PWAのreceiverはchannelを識別せずユーザー単位でpendingを取得するため、複数channel間でもpendingは一件に制限する。この更新はneon-httpの制約に合わせ、単一SQLとDB制約でアトミックに行う。`INSERT`は既存pendingを更新するdata-modifying CTEの`RETURNING`結果へ明示的に依存させる。同一ユーザーへの同時リクエストでpendingのpartial unique indexと競合した場合は、その制約の`23505`に限りSQL全体を一度だけ再試行する。
