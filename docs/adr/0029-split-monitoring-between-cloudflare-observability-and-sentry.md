...
送る内容はログと同じ線に揃える。request body（レシピ本文、Stripeのpayload）、cookie、URLのqueryは送らない。Workerの外部fetchのbreadcrumbは取り込み元のURLや署名付きURLを含むので、originだけを残す。これはログに`sourceHost`だけを残すのと同じ扱いである。利用者はidだけを載せる。
