...
TikTok URL import では、`https://www.tiktok.com/embed/v2/{id}` の HTML に埋め込まれた `__FRONTITY_CONNECT_STATE__` JSON を単一の取得経路として使用します。取得対象は caption (`itemInfos.text`)、投稿者 (`authorInfos.uniqueId` / `nickName`)、カバー画像 (`itemInfos.coversOrigin` / `covers`)、photo carousel の画像 (`imagePostInfo.displayImages[].urlList`) です。動画の音声、映像、字幕は解析しません。
...
canonical URL は `authorInfos.uniqueId` から `https://www.tiktok.com/@{uniqueId}/{video|photo}/{id}` を組み立てます。URL パス上の `@user` は投稿者と一致していなくても TikTok 側で解決されるため、embed が返す値を正とします。video と photo の区別も同じ理由で URL パスではなく `displayImages` の有無から決めます。
