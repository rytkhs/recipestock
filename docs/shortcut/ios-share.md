...
配布後のShortcutは更新できない。判断ロジックはサーバーに置き、Shortcutは受け取った文字列を表示するだけにする（ADR 0008）。アクション列はADR 0025で確定させた形であり、画像共有への対応を含めて、これ以上増やさない。
...
4. **URLの内容を取得**
   ...
   - 本文: JSON、`input` = 手順2の結果、`deviceName` = 手順3の結果
5. **通知を表示** — タイトル `notice.title`、本文 `notice.body`
6. **もし** `notice.openUrl` が `https://` を含む **なら** — **URLを開く** `notice.openUrl`

連携トークンは、Shortcut追加時のインポート質問でユーザーが貼り付ける。判断ロジックはゼロ、分岐は手順6の1つだけである。

受け付ける型を「すべて」にしない。画像を共有したときにこのShortcutが共有シートへ現れないことが、`no_url_in_input`を利用者に見せないための設計である。画像とスクリーンショットからの取り込みは、このShortcutを作り直さず、受け付ける型が画像の別Shortcutとして配る（ADR 0025）。

手順3のデバイス名は、設定画面の連携済み端末一覧に出す表示名である。日本語のデバイス名は珍しくなく、HTTP headerの値はlatin-1を前提とするため、headerではなくJSON bodyで運ぶ。

`notice.body`は常に存在し、2行目を出さないreasonでは空文字になる。手順5はbodyの有無を分岐せず、空文字のときはタイトルだけの1行通知になる。

手順6に「値がある」を使わない。Shortcutsの「値がある」は空でないことの判定ではなく、辞書から取り出した値は空でも「値がある」側へ流れる。JSONの`null`が「値なし」「空文字」「文字列`null`」のどれになるかはAppleが仕様として公開しておらず、iOSの版で変わりうる。配布後のShortcutは更新できないため、この未定義の挙動に分岐を賭けない。`openUrl`は必ず絶対URLなので、`https://`を含むかどうかで判定すれば`null`がどう化けても必ず偽になる。この理由は手順6にだけ当てはまり、`deviceName`はShortcut側で分岐に使わないので影響しない。
...
`deviceName`は連携済み端末を見分けるための補助情報で、初回の共有だけが保存に使われる。取り込みを失敗させてはならないため、型や長さが想定と違う値は`malformed_request`にせず、その場で捨てて`input`の処理を続ける。保存時は前後の空白を落として60文字で切る。デバイス名には利用者の実名が入りやすいので、DBには保存しても監視ログへは出さない。

連携トークンは`rssc_`と乱数25文字（`[A-Za-z0-9_-]`）の計30文字で、150bitである。DBにはSHA-256のhashだけを保存し、末尾4文字を`tokenSuffix`として平文で持ち、設定画面の連携済み端末一覧に出す。suffixは平文で公開する分だけ実効エントロピーを削るため、長くしない。認証の前に`shortcutCredentialTokenSchema`で形式を検査し、合わないトークンはhash照合のDBアクセスへ到達させない（ADR 0025）。
...
トークンの発行は連携の完了ではない。契約に合うrequestが認証を初めて通過した時点で`verified_at`を確定し、同じ更新で`deviceName`を端末名として保存する。設定画面はこれを「連携済み」として表示し、未使用のcredentialを「連携待ち」と区別する（ADR 0025）。

確定は取り込みの結果と切り離す。`no_url_in_input`や`recipe_limit_exceeded`で終わった共有でも確定する。例外は`malformed_request`で、これは配布したShortcutが壊れていることの検知に使う`reason`であるため、確定させない。

書き込みは初回の1回だけである。`verified_at`は認証のSELECTが返すので、確定済みのcredentialでは追加のDBアクセスが発生しない。
...
この確認は`受け取るもの`を`すべて`にした状態で行ったものであり、配布する構成ではない。配布構成では画像が共有シートの候補から外れるため、画像を共有してファイル名が`input`へ入る経路自体が発生しない。

受け取る型やこの前提を変える場合は、同じ確認をやり直してから契約を決める。

## 分岐の実機確認

配布前に、`openUrl`が`null`のreason（`created`）と`openUrl`を持つreason（`unauthorized`）の両方を実機で通し、前者で遷移も失敗も起きないこと、後者で遷移することを確認する。手順6はShortcut唯一の分岐であり、配布後は修正できない。

あわせて次を実機で確認する。

- 日本語を含むデバイス名が`deviceName`として届き、初回の共有のあとに設定画面の一覧へその名前で表示されること
- 画像だけを共有したとき、このShortcutが共有シートの候補に現れないこと
