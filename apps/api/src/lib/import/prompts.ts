...
- note: Include only recipe-specific notes, tips, storage guidance, substitutions, or serving guidance supported by the evidence. Copy note text verbatim from the evidence. Do not summarize multiple note-like passages into one sentence. If multiple distinct notes are present, preserve their wording and order, separated only by newlines. Do not include provenance or uncertainty commentary.
